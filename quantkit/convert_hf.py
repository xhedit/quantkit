...
from quantkit.convert import LlamaHfVocab
...
class Model:
    ...
    def get_vocab_base_pre(self, tokenizer) -> str:
        ...
        if chkhsh == "c136ed14d01c2745d4f60a9596ae66800e2b61fa45643e72436041855ad4089d":
            # ref: misc tunes with different tokenizers
            res = "llama-bpe"
...
def do_gguf_conversion(model: PosixPath, output: str, out_type: str, vocab_dir: str, vocab_type: str, context: int, pad_vocab: bool, concurrency: bool, big_endian: bool) -> None:
    ...
    ftype = ftype_map[out_type]
    print(f"out_type: {out_type}")
    print(f"Loading model: {model} / Making {output}")

    hparams = Model.load_hparams(model)

    with torch.inference_mode():
        model_class = Model.from_model_architecture(hparams["architectures"][0])
        model_instance = model_class(Path(model), ftype, Path(output), big_endian, False, False)
        #model_instance = model_class(Path(model), ftype, fname_out, big_endian, args.use_temp_file, args.no_lazy)
        ...
        print(f"Exporting model to '{model_instance.fname_out}'")
        model_instance.write()
